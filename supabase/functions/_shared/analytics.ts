// Server-side port of src/lib/analytics.ts — kept dependency-free so it
// runs identically in Deno (Edge Functions) and in the browser.
...
export function buildProximityInsight(
  ...
  const pick = (p: PropertyRow) => (metric === 'price' ? p.price_inr : p.price_per_sqft);
...
export function computeDerivedMetrics(properties: PropertyRow[]): DerivedMetrics {
  ...
  const calcJrei = (ps: PropertyRow[]): number => {
    ...
    const proximityUtil = withinCount / n;
    ...
    const proximityComp = 0.5 + proximityUtil;
